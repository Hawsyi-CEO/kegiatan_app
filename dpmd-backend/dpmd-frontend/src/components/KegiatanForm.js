...
const KegiatanForm = ({ kegiatan: initialKegiatan, onClose, onSuccess }) => {
    ...
    const handleBidangChange = (index, e) => {
    ...
    const fetchPersonilByBidang = async (idBidang) => {
        if (!allPersonilByBidang[idBidang]) {
            try {
                const response = await axios.get(`${API_URL}/personil-by-bidang/${idBidang}`);
    ...
    const handleSubmit = async (e) => {
        ...
        const method = initialKegiatan ? 'PUT' : 'POST';
        const url = initialKegiatan ? `${API_URL}/kegiatan/${initialKegiatan.id_kegiatan}` : `${API_URL}/kegiatan`;
        ...
        try {
            ...
            // Logika baru untuk menangani konflik jadwal
